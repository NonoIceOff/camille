...
client.on(Events.InteractionCreate, async (interaction) => {
    ...
    if (interaction.isChatInputCommand()) {
        ...
        if (interaction.commandName === "help") {
            ...
            text =
                text + "**/coinslead** : *classement des coins du serveur* \n";
            text =
                text +
        ...
        if (interaction.commandName === "stats") {
            ...
            if (membersdico[interaction.member.id]) {
                ...
                monnaie =
                    Math.round(
                        membersdico[interaction.member.id]["esheep"] * 1000
                    ) / 1000;
                max_xp_to_level =
                    5 *
                        Math.pow(
                            membersdico[interaction.member.id]["niveau"],
                            2
                        ) +
            ...
            const exampleEmbed = new EmbedBuilder()
                ...
                .setDescription(
                    "Toutes vos statisques, au même endroit, c'est ICI."
                )
                .addFields(
                    {
                        name: "Niveau :",
        ...
        if (interaction.commandName === "vote") {
            if (
                ...
                client.channels.cache.get(
                    constantIDs.channels.bot[+options.test]
                )
            ) {
                let file = editJsonFile("./infos.json");
                ...
                if (
                    ...
                    desc =
                        "\n*Il vous reste " +
                        (max_votes -
                            membersdico["voted"][interaction.user.id] -
                            1) +
                        " vote.*";
                } else {
                ...
                if (membersdico["voted"]["start"] === 1) {
                    if (membersdico["voted"][interaction.user.id] < max_votes) {
                        ...
                        const exampleEmbed = new EmbedBuilder()
                            .setColor(10181046)
                            .setTitle(
                                ":ballot_box:  __**Vote comptabilisé !**__"
                            )
                            .setDescription(
                                "Merci <@" +
                                    interaction.user +
                        ...
                        const exampleEmbed = new EmbedBuilder()
                            .setColor(10038562)
                            .setTitle(
                                ":warning:  __**Vote non comptabilisé.**__"
                            )
                            .setDescription(
                                "Vous avez déja épuisé tous vos votes."
                            );
                        //interaction.channel.permissionOverwrites
                        //    .edit(interaction.user.id, { SendMessages: false});
                        interaction.reply({
        ...
        if (interaction.commandName === "quests") {
            ...
            if (
                quests_completed === quests["Level " + level.toString()].length
            ) {
            ...
            pourcentage =
                pourcentage / quests["Level " + level.toString()].length;
        ...
        if (interaction.commandName === "fightlead") {
            ...
            for (let i = 1; i <= membersnbr; i++) {
                ...
                if (user.includes("IA") == false) {
                    let usered = await client.users.fetch(
