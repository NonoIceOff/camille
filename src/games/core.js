...
function init(user, game) {
    if (games[game]) {
        ...
        user.send({
            content: `Déso, le jeu \`${game}\` n'est pas encore implémenté, ça arrivera un jour 🤞`,
        });
    }
}
...
function onMessage(message) {
    if (!message.guild && !message.author.bot) {
        if (playingGames[message.author.id])
            playingGames[message.author.id].onMessage(message);
    }
}
    ...
    if (!interaction.guild && !interaction.isChatInputCommand()) {
        ...
        if (path[0] === "game") {
            if (!playingGames[interaction.user.id]) {
                interaction.reply({
                    content:
                        "Alors... Joue à un jeu déjà, après on vera si tu peux jouer.",
                    ephemeral: true,
                });
            ...
            if (playingGames[interaction.user.id].name == path[1]) {
                if (interaction.isButton()) {
                    playingGames[interaction.user.id].onButton(
                        interaction,
                        path
                    );
                } else if (interaction.isSelectMenu()) {
                    playingGames[interaction.user.id].onSelectMenu(
                        interaction,
                        path
                    );
                }
                ...
                interaction.reply({
                    content:
                        "Heu... Je crois que tu peux pas faire ça, pas sûr, mais on me dit dans l'oreillette que c'est un jeu auquel tu jouais avant.",
                    ephemeral: true,
                });
            }
        }
